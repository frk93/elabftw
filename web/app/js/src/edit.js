    ...
    init: function() {
      ...
      this.on("complete", function(answer) {
        ...
        if (
          ...
          $("#filesdiv").load(
            "?mode=edit&id=" + $("#info").data("id") + " #filesdiv",
            function() {
  ...
  $(document).ready(function() {
    ...
    if (
      ...
      let bodyRecovery = $("<div></div>", {
        ...
        html:
          "Recovery data found (saved on " +
          localStorage.getItem("date") +
          '). It was probably saved because your session timed out and it could not be saved in the database. Do you want to recover it?<br><button class="button recover-yes">YES</button> <button class="button button-delete recover-no">NO</button><br><br>Here is what it looks like: ' +
    ...
    function getListFromMolFiles() {
      ...
      $.get("app/controllers/AjaxController.php", {
        ...
        mols.forEach(function(mol, index) {
          listHtml +=
            '<li style="color:#29aeb9" class="clickable loadableMolLink" data-target="app/download.php?f=' +
            mols[index][1] +
            '">' +
            mols[index][0] +
            "</li>";
    ...
    // Creating a Dialog that can be called from the TinyMCE
    var dialogConfig = {
      title: "My Dialog",
      body: {
        type: "panel",
        items: [
          {
            type: "htmlpanel",
            html: "This is my first dialog with TinyMCE"
          }
        ]
      },
      buttons: [
    ...
    tinymce.init({
      ...
      plugins:
        "autosave table searchreplace code fullscreen insertdatetime paste charmap lists advlist save image imagetools link pagebreak mention codesample hr template",
      pagebreak_separator: "<pagebreak>",
      toolbar1:
        "undo redo | styleselect bold italic underline | alignleft aligncenter alignright alignjustify | superscript subscript | bullist numlist outdent indent | forecolor backcolor | charmap | codesample | link | save | myCustomToolbarButton",
      ...
      setup: function(editor) {
        editor.ui.registry.addButton("myCustomToolbarButton", {
          text: "Test Button",
          onAction: () => editor.windowManager.open(dialogConfig) // Implementation of dialog config is above
        });
        editor.addShortcut("ctrl+shift+d", "add date at cursor", function() {
          addDateOnCursor();
        });
